...
export function isMailingPreferenceRequest(text, configuredKeywords = '') {
  ...
  // הפועל בכל צורותיו המדוברות: הסר/הסירו, תסיר/תסירו, תוריד/תורידו וכו׳ —
  // «בבקשה תסירו אותנו מרשימת התפוצה» היא הבקשה שהניסוח הקודם פספס,
  // והבוט ענה עליה «מעביר לצוות» (2026-08-16).
  const removeVerb = '(?:הסר(?:ו)?|הסיר(?:ו)?|תסיר(?:ו)?|להסיר|הפסק(?:ו)?|הפסיק(?:ו)?|תפסיק(?:ו)?|להפסיק|בטל(?:ו)?|תבטל(?:ו)?|לבטל|הורד(?:ו)?|הוריד(?:ו)?|תוריד(?:ו)?|להוריד)';
  const removeObject = '(?:\\s+(?:אותי|אותנו|אותם))?(?:\\s+לשלוח)?(?:\\s+(?:לי|לנו|להם))?';
  const removeTarget = '\\s+(?:את\\s+)?מ?(?:ה)?(?:דיוור|הודעות|עדכונים|פרסומים|תפוצה|רשימ(?:ה|ות|ת\\s+(?:ה)?תפוצה))';
  return new RegExp(`${removeVerb}${removeObject}${removeTarget}`, 'u').test(raw)
