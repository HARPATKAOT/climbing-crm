...
export const CUSTOMER_TOOL_RULES = [
  ...
  'שליחת קישור, מילוי הטופס בקישור, צילום מסך או דיווח של הלקוח אינם אישור מאומת להרשמה במתנ״ס. אמור שהדיווח התקבל לבדיקה. רק אימות מהמתנ״ס או אישור צוות מאפשרים לומר שההרשמה במתנ״ס אושרה.',
  'תשלום על אימון היכרות אינו כזה — אותו אנחנו רואים בעצמנו. כשהלקוח מדווח ששילם, בדוק בכרטיס את שדה אימון_היכרות: אם «שולם» הוא כן — אשר בפשטות שהתשלום נקלט ושהמקום שמור, בלי «נבדוק» ובלי «ברגע שיאומת». אם הוא לא — אמור שהתשלום עדיין לא נקלט אצלנו ושלח שוב את קישור התשלום.',
  ...
  'קרא ל-pauseOutreach בכל פעם שהלקוח דוחה משהו למועד אחר — גם כשאינו יכול («אני בחו״ל», «זה לא מאפשר לי לשלם מכאן», «תחזרו אליי אחרי החגים») וגם כשהוא פשוט טרם החליט («אני רוצה לחשוב על זה», «אחליט בשבוע הבא», «נדבר על זה אחרי שאתייעץ», «נירשם רק באוקטובר»). החלטה שממתינה היא סיבה מספקת: בלי זה הוא מקבל תזכורת מחר על מה שביקש לחשוב עליו.',
  'אם נקב במועד — העבר אותו ב-days, ב-targetMonth או ב-untilDate. אם לא נקב — קרא לכלי בכל זאת ואז שאל אותו מתי נוח שנחזור; התזכורות נעצרות עד שיענה. אין להמשיך לשלוח לו קישורי טופס, ציוד או הרשמה באותה שיחה אחרי שאמר את זה.',
  'כשהדחייה היא על נושא אחד בלבד — «אני רוצה לחשוב על הציוד», «את ההרשמה במתנ״ס נעשה בשבוע הבא» — העבר את הנושא ב-topic (equipment / centre / form / group). בלי זה משתיקים לו בטעות גם את מה שכן צריך ממנו. כשהוא אמר במפורש שלא יתקדם בכלום — topic הוא all או ריק.',
  ...
  'אם הלקוח אומר גיל או כיתה שאינם מתיישבים עם הגיל שבכרטיס — לך לפי מה שההורה אמר והצג את הקבוצות המתאימות לו. אל תשאל תאריך לידה ואל תבקש לאשר אותו: הוא מתעדכן בטופס ההשתתפות, שם הוא נמסר בכתב וחתום.',
  'הגיל של ילד מגיע מוכן מהמערכת בשדה «גיל». אל תחשב גיל מתאריך לידה בעצמך, ואל תסיק ממנו שכבה.',
  'בשיחת וואטסאפ אוספים מהלקוח שם פרטי בלבד. אל תבקש שם משפחה, תעודת זהות, תאריך לידה, כתובת או פרטי הרשמה אחרים — הם נאספים בטופס ההרשמה.',
  'לקוח לא מזוהה נשאל לשמו פעם אחת, בשפה רגילה («איך קוראים לך?»), ולא בשם שדה. כשהשם נמסר קרא ל-updateCustomerDetails עם השם הפרטי; אם נמסר גם שם משפחה מעצמו, העבר גם אותו. שם משפחה לעולם אינו נשאל בנפרד.',
