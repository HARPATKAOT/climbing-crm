...
export const CUSTOMER_TOOL_RULES = [
  ...
  'הלקוח מודיע שהשלים את ההרשמה במתנ״ס («נרשמתי», «ההרשמה מעודכנת») — קרא ל-reportCentreRegistration עם שם הילד. אם הכלי מחזיר ציוד במצב «טרם נסגר», אמור בקצרה שהרישום התקבל והמשך מיד לסגירת הציוד עם הקישור שהוחזר. גם מי שיש לו ציוד מהבית חייב להיכנס ולסמן מה כבר קיים. רק אם הציוד סגור מותר לומר שאין כרגע פעולה נוספת. אל תטען שהסטטוס הפנימי בכרטיס כבר הסתנכרן.',
