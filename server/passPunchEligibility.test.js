...
const validDecl = { studentId: 's1', signedDate: '2026-09-01', waiverAccepted: true };
...
test('הכל בתוקף — מותר לנקב', () => {
  const reason = passPunchBlockReason(
    { student, declarations: [validDecl], tests: [safetyTest('2026-12-01')] },
    NOW
  );
...
test('בלי הצהרת בריאות — חסום', () => {
  ...
  assert.match(reason, /לא נחתמה הצהרת בריאות והסרת אחריות/);
});

test('בלי מבחן אבטחה — חסום', () => {
  const reason = passPunchBlockReason({ student, declarations: [validDecl], tests: [] }, NOW);
...
test('מבחן אבטחה שפג — חסום עם תאריך התפוגה', () => {
  ...
  const reason = passPunchBlockReason(
    { student, declarations: [validDecl], tests: [safetyTest('2026-06-01')] },
    NOW
  );
