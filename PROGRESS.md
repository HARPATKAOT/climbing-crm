...
## סבב משוב 2 (16.08) — בוצע
- **תיקון OOM**: skipSync+deferFlush — הריצה הלילית לא יורה יותר אלפי כתיבות צפות; שדרוג השרת ל-2GB.
- **באג "החוב שנעלם"**: חוב פתוח מוצג תמיד (מלאי, לא תזרים) בכל שלושת המסלולים; חשבונית iCount לא-פרועה = חוב.
- **מרכז הוצאות**: שורה אחת פר הוצאה, מקור מתויג, חשבונית יש/אין באדום, תיוג AI (ג'מיני, סף 0.7, חוק/אדם גוברים), שליחה לרו״ח במייל + חבילה חודשית.
- **הכנסות**: תגיות אמצעי תשלום צבעוניות, אייקונים בפילטרים, פילטר סוג מסמך, KPI לחיצים עם דיפ-לינק.
- **סקירה**: toolbar נקי (בלי "iCount חי"), 4 קוביות בשורה, תת-תצוגת רווחיות עם טבלאות ברורות לחוגים/אירועים/עובדים.
...
## איך לבדוק ידנית
- טסטים: `cd server && npm run test:finance` (109 טסטי finance) או `npm test` (1,265). הכול עובר נכון ל-2026-08-15.
- מסך בפיתוח (דגלים דלוקים אוטומטית מחוץ לפרודקשן): `npm run dev` בשורש → `/reports`:
  - טאב **מרכז התאמות**: העלאת חשבונית PDF → נקלטת עם שדות; "הרצת מנוע ההתאמה"; אישור הצעה עם מקש A.
  - טאב **רווחיות ותזרים**: toggle מזומן/צבירה; לחיצה על עמודת חודש במפל → drill-down לשורות המקור.
  - טאב **התאמות ואיכות נתונים**: פאנל בריאות הסנכרון למעלה.
  - טאב **קליטה והתאמה** (הישן): ייבוא CSV מזין עכשיו גם את המרכז החדש (finance_center בסיכום).
- ג'וב לילי ידנית: `POST /api/finance/nightly-scheduled` עם כותרת `x-cron-secret` — מריץ outbox, יישוב, ledger, תחזית.
- הפעלה בפרודקשן (אחרי דיפלוי Render): דגל-דגל דרך env, למשל `FINANCE_FLAG_LEDGER=1`, או שורת settings. הכול כבוי כברירת מחדל.
