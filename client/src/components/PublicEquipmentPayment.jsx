  ...
  return (
    <div className="eq-pay-page">
                          ...
                          <div key={type} className={`eq-pay-item ${checked ? 'is-on' : ''} ${isOwn ? 'is-own' : ''} ${isPaid ? 'is-paid' : ''}`}>
                            ...
                            {(isUnpaid || isOwn) && (
                              <button type="button" className={`eq-pay-own ${isOwn ? 'is-cancel' : ''}`} disabled={owning === busyKey} aria-pressed={isOwn} onClick={() => setItemOwnership(member, type, !isOwn)}>
                                {owning === busyKey ? (isOwn ? 'מבטל…' : 'שומר…') : (isOwn ? 'ביטול' : 'כבר יש לנו')}
                              </button>
                            )}
                          </div>
                        );
      ...
      <style>{`
        ...
        @media(max-width:560px){.eq-pay-page{padding:10px 8px 28px}.eq-pay-header,.eq-pay-card{border-radius:17px}.eq-pay-brand{font-size:25px;gap:7px}.eq-pay-brand img{width:94px;height:94px}.eq-pay-required{align-items:flex-start;padding:13px}.eq-pay-required strong{font-size:16px}.eq-pay-required span{font-size:13px}.eq-pay-item{flex-wrap:wrap}.eq-pay-item-details{min-width:calc(100% - 38px)}.eq-pay-own{margin-inline-start:32px}.eq-member{align-items:flex-start;flex-wrap:wrap}.eq-member>b{width:100%;margin-inline-start:32px}.eq-frame-summary{flex-wrap:wrap}}
      `}</style>
    </div>
  );
}
