...
**החיבור נופל כל ~7 ימים גם אחרי פרסום למצב חי**  
מפתח שנוצר כשמסך ההסכמה היה במצב Testing פג אחרי 7 ימים, גם אם אחר כך העברתם ל־In production.  
אחרי הפרסום לחצו „חיבור מחדש” פעם אחת — רק מפתח חדש שנולד במצב חי נשאר יציב.  
ודאו שבקונסולת גוגל תחת OAuth consent screen עדיין כתוב In production.

**החיבור מתנתק שוב ושוב באותו יום**  
אל תריצו שרת מקומי (`crm-api` / `npm run dev` בתיקיית server) מול אותו מסד כמו השרת החי בלי לדעת.  
שרת מקומי עם מפתח ישן בזיכרון היה דורס חיבור חדש שנשמר מהמערכת החיה.  
הקוד עכשיו: (1) תמיד קורא מהמסד לפני שמירת הגדרות גוגל, (2) לא מריץ סנכרון ברקע מקומי כברירת מחדל.  
לפני חיבור מחדש במערכת החיה עדיין מומלץ: `npx pm2 stop crm-api`.
